...
上线状态（2026-09-15）：公开 GitHub 仅运行公共行情与固定合成账户，不读取真实账户。
...
全球/行业/基本面及完整多周期来源未齐，仍为降级分析；账本已接入可选的私有观察，真实验收未开始。
...
`.github/workflows/public-trial.yml` 是当前公开测试入口：四时点工作日调度、固定公共
指数和合成账户，产物写入独立的 `public-test-data` 分支。它不读取真实账户或私有仓库，
...
`full_analysis_ready` 仍为 false；缺失全球、行业、基本面和多周期确认时保留 WAIT / +0%。
